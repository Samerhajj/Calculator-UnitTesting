...
namespace LoreSoft.MathExpressions
{
    ...
    public class FunctionExpression : ExpressionBase
    {
        ...
        private static readonly string[] oneArgumentMathFunctions = new string[]
            {
                "abs", "acos", "asin", "atan", "ceiling", "cos", "cosh", "ln",
        ...
        public double Execute(double[] numbers)
        {
            ...
            if ((function == "Tan" || function == "Sin" || function == "Cos") && Global.isDegree)
            {
                List<double> newnumber = new List<double>();
                double newnam;

                foreach(var number in numbers)
                {
                    newnam = number * Math.PI / 180;
                    newnumber.Add(newnam);
                    
                }
               ;
                Array.Copy(newnumber.ToArray(), parameters, numbers.Length);
